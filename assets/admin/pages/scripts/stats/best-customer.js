...
$(document).ready(function () {
    ...
    $("#submenu_bestcustomers").addClass("active");

    InitDT("#dtBestCustomer");
});
...
function InitDT(element) {
    ...
    var e = $(element).dataTable({
        ...
        sAjaxSource: "/api/services.asmx/admin_dtBestCustomer",
        fnInitComplete: function () {
            this.fnSetFilteringDelay(500);
        },
        aoColumns: [{ mDataProp: "Email" },
                    { mDataProp: "Name" },
                    { mDataProp: "Quantity" },
                    { mDataProp: "Amount" }
        ],
        ...
        fnRowCallback: function (nRow, aaData, dIndex, rIndex) {
            ...
            var v = "";
            v += '<span class="format-money">' + aaData.Amount + '</span>';
            $("td:eq(3)", nRow).empty().append(v);
