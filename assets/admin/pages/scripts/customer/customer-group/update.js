...
$(document).ready(function () {
    ...
    if (queryString("id")) {
        $("#HiddenIDCustomerGroup").val(queryString("id"));
        Preload(+$("#HiddenIDCustomerGroup").val());
    }
    else
...
function Preload(id) {
    ...
    $.ajax({
        ...
        data: JSON.stringify({
            ...
            m: "upreload",
            data: {
                IDCustomerGroup: $("#HiddenIDCustomerGroup").val()
            }
        }),
        success: function (result) {
            if (result.success) {
                $.each(result.data.CustomerGroup, function (indexInArray, valueOfElement) {
                    if (indexInArray == "Active" || indexInArray == "IsPoint") {
                        if (valueOfElement == true)
                            $("[name=" + indexInArray + "]").bootstrapSwitch("state", true);
                        else if (valueOfElement == false)
                            $("[name=" + indexInArray + "]").bootstrapSwitch("state", false);
                    }
                    else
...
function GroupUpdateValidation() {
...
function SubmitData() {
    ...
    data["IDCustomerGroup"] = +$("#HiddenIDCustomerGroup").val();
