...
$(document).ready(function () {
    ...
    //ReloadShippingByCarrier(queryString("idcarrier"));
    InitDT("#dtShipping", queryString("idcarrier"));
});

function InitDT(element, idCarrier) {
    ...
    var e = $(element).dataTable({
        ...
        sAjaxSource: "/api/services.asmx/dtShip",
        ...
        aoColumns: [
            { mDataProp: "CityName", sWidth: "30%" },
            { mDataProp: "DistrictName", sWidth: "30%" },
            { mDataProp: "Price", sWidth: "20%" },
            { mDataProp: "IDShipping", sWidth: "20%" },
        ],
        aoColumnDefs: [{
            //bSortable: false,
            //aTargets: [4]
        ...
        fnRowCallback: function (nRow, aaData, dIndex, rIndex) {
            var textbox = '<input type="text" class="form-control price-' + aaData.IDShipping + '" value="' + aaData.Price + '" />';
            $("td:eq(2)", nRow).html(textbox);

            var button = '<button data-id="' + aaData.IDShipping + '" class="btn btn-xs btn-blue btn-update">Save</button>';
        ...
        fnDrawCallback: function (e) {
            $(".btn-update").click(function (e) {
                ...
                REST.sendRequest({
                    'c': 'beship',
                    'm': 'uprice',
                    'data': { 'IDShipping': id, 'Price': price }
                });
            });
...
function ReloadShippingByCarrier(IDCarrier) {
    REST.onSuccess = function (data) {
        var result = data.d;
        if (result.success) {
    ...
    REST.sendRequest({
        'c': 'beship',
        'm': 'rbycar',
        'data': { 'IDCarrier': parseInt(IDCarrier) }
    });
}