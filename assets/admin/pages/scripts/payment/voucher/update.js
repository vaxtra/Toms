...
$(document).ready(function () {
    ...
    $("#submenu_voucher").addClass("active");

    if (queryString("id")) {
        $("#HiddenIDVoucher").val(queryString("id"));
        Preload(+$("#HiddenIDVoucher").val());
    }
    else
...
function Preload(id) {
    REST.onSuccess = function (result) {
        if (result.d.success) {
            ...
            var _idcustomer = result.d.data.Voucher.IDCustomer;
            ...
            for (var i = 0; i < dataCustomer.length; i++) {
                ...
                if (dataCustomer[i].IDCustomer == _idcustomer)
                    selectedData = {
                        id: dataCustomer[i].IDCustomer,
                        text: dataCustomer[i].FirstName + ' ' + dataCustomer[i].LastName + ' (' + dataCustomer[i].Email + ')'
                    };
            ...
            $.each(result.d.data.Voucher, function (indexInArray, valueOfElement) {
                if (indexInArray == "VoucherType") {
                    if (valueOfElement == "percent")
                        $("[name=VoucherType]").bootstrapSwitch("state", true);
                    else if (valueOfElement == "amount")
                        $("[name=VoucherType]").bootstrapSwitch("state", false);
                }
                else if (indexInArray == "StartDate" || indexInArray == "EndDate")
                    $("[name=" + indexInArray + "]").val(moment(valueOfElement).format("DD MMMM YYYY HH:mm"));
                else
...
function VoucherUpdateValidation() {
    var e = $("#form_voucher");
    var t = $(".alert", e);
...
function SubmitData() {
    ...
    data["IDVoucher"] = +$("#HiddenIDVoucher").val();
