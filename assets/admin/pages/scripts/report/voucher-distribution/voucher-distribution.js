...
$(document).ready(function () {
    ...
    $("#submenu_voucher-distribution").addClass("active");
    InitDT("#dtVoucherDis");
    InitDateRange();
});
...
function InitDT(element) {
    ...
    var e = $(element).dataTable({
        ...
        sAjaxSource: "/api/services.asmx/admin_dtVoucherDistribution",
        fnInitComplete: function () {
            this.fnSetFilteringDelay(500);
        },
        aoColumns: [{ mDataProp: "Number", sWidth: "5%" },
                    { mDataProp: "Voucher", sWidth: "20%" },
                    { mDataProp: "Used", sWidth: "20%" },
                    { mDataProp: "TotalDiscount", sWidth: "20%" },
        ],
        aoColumnDefs: [{
            bSortable: false,
            aTargets: [3]
        ...
        fnRowCallback: function (nRow, aaData, dIndex, rIndex) {
            ...
            var s = "";
            s += '<span class="format-money">' + aaData.TotalDiscount + '</span>';
            $("td:eq(3)", nRow).empty().append(s);
        },
...
function InitDateRange() {
    ...
    function (start, end) {
        ...
        //FullFilter();
        InitDT_Filter("#dtVoucherDis", $(".input-mini[name=daterangepicker_start]").val(), $(".input-mini[name=daterangepicker_end]").val());
...
function InitDT_Filter(element, startDate, endDate) {
    ...
    var e = $(element).dataTable({
        ...
        bRetrieve: true,
        sPaginationType: "bootstrap",
        sAjaxSource: "/api/services.asmx/admin_dtVoucherDistribution_FilterDate",
