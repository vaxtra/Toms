...
$(document).ready(function () {
    ...
    $("#submenu_product-distribution").addClass("active");
    InitDT("#dtProductDis");
    InitDateRange();
});
...
function InitDT(element) {
    ...
    var e = $(element).dataTable({
        ...
        sAjaxSource: "/api/services.asmx/admin_dtProductDistribution",
        fnInitComplete: function () {
            this.fnSetFilteringDelay(500);
        },
        aoColumns: [{ mDataProp: "Number", sWidth: "5%" },
                    { mDataProp: "Product", sWidth: "20%" },
                    { mDataProp: "Combination", sWidth: "15%" },
                    { mDataProp: "TotalOrders", sWidth: "5%" },
                    { mDataProp: "PricePerUnit", sWidth: "20%" },
                    { mDataProp: "QtyPerDay", sWidth: "20%" },
        ...
        fnRowCallback: function (nRow, aaData, dIndex, rIndex) {
            ...
            var c = "";
            for (var i = 0; i < aaData.Combination.length; i++) {
                c += '<span>' + aaData.Combination[i].Name + '&nbsp;&nbsp; - &nbsp;&nbsp;' + aaData.Combination[i].Quantity + '</span><br/><br/>';
            }
            $("td:eq(2)", nRow).empty().append(c);
            var o = "";
            o += '<span class="format-money">' + aaData.PricePerUnit + '</span>';
            $("td:eq(4)", nRow).empty().append(o);
            var s = "";
            s += '<span class="format-money">' + aaData.TotalSales + '</span>';
            $("td:eq(6)", nRow).empty().append(s);
        },
...
function InitDateRange() {
    ...
    function (start, end) {
        ...
        //FullFilter();
        InitDT_Filter("#dtProductDis", $(".input-mini[name=daterangepicker_start]").val(), $(".input-mini[name=daterangepicker_end]").val());
...
function InitDT_Filter(element, startDate, endDate) {
    ...
    var e = $(element).dataTable({
        ...
        bRetrieve: true,
        sPaginationType: "bootstrap",
        sAjaxSource: "/api/services.asmx/admin_dtProductDistribution_FilterDate",
