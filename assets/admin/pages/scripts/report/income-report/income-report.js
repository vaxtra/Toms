...
$(document).ready(function () {
    ...
    $("#submenu_income-report").addClass("active");
    InitDT("#dtReport");
    PreloadReport();
    InitDateRange();
});
...
function InitDT(element) {
    ...
    var e = $(element).dataTable({
        ...
        sAjaxSource: "/api/services.asmx/admin_dtReport",
        fnInitComplete: function () {
            this.fnSetFilteringDelay(500);
        },
        aoColumns: [{ mDataProp: "Number", sWidth: "5%" },
                    { mDataProp: "Date", sWidth: "15%" },
                    { mDataProp: "TotalRegistrant", sWidth: "16%" },
                    { mDataProp: "TotalOrders", sWidth: "16%" },
                    { mDataProp: "TotalItems", sWidth: "16%" },
                    { mDataProp: "TotalSales", sWidth: "16%" },
                    { mDataProp: "TotalSalesVoucher", sWidth: "16%" },
        ],
        aoColumnDefs: [{
            bSortable: false,
            aTargets: [6]
        ...
        fnRowCallback: function (nRow, aaData, dIndex, rIndex) {
            ...
            var v = "";
            v += '<span class="format-money">' + aaData.TotalSalesVoucher + '</span>';
            $("td:eq(6)", nRow).empty().append(v);
        },
...
function PreloadReport()
{
    ...
    REST.sendRequest({
        ...
        'm': 'preload',
        'data': {
            'RequestData': ["TotalCustomer", "TotalOrder", "TotalSales", "TotalItemsSold", "TotalSalesVoucher"],
            'startDate': null,
            'endDate': null
        }
    });
...
function InitDT_Filter(element, startDate, endDate) {
    ...
    var e = $(element).dataTable({
        ...
        bRetrieve: true,
        sPaginationType: "bootstrap",
        sAjaxSource: "/api/services.asmx/admin_dtReport_FilterDate",
