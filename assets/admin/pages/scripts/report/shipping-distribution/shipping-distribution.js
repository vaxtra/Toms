...
$(document).ready(function () {
    ...
    $("#submenu_shipping-distribution").addClass("active");
    InitDT("#dtShippingDis");
    InitDateRange();
});
...
function InitDT(element) {
    ...
    var e = $(element).dataTable({
        ...
        sAjaxSource: "/api/services.asmx/admin_dtShippingDistribution",
        fnInitComplete: function () {
            this.fnSetFilteringDelay(500);
        },
        aoColumns: [{ mDataProp: "Number", sWidth: "5%" },
                    { mDataProp: "ShippingMethod", sWidth: "20%" },
                    { mDataProp: "TotalOrders", sWidth: "20%" },
                    { mDataProp: "TotalSales", sWidth: "20%" },
                    { mDataProp: "TotalShippingCost", sWidth: "20%" },
        ],
        aoColumnDefs: [{
            bSortable: false,
            aTargets: [4]
        ...
        fnRowCallback: function (nRow, aaData, dIndex, rIndex) {
            ...
            var s = "";
            s += '<span class="format-money">' + aaData.TotalShippingCost + '</span>';
            $("td:eq(4)", nRow).empty().append(s);
        },
...
function InitDateRange() {
    ...
    function (start, end) {
        ...
        //FullFilter();
        InitDT_Filter("#dtShippingDis", $(".input-mini[name=daterangepicker_start]").val(), $(".input-mini[name=daterangepicker_end]").val());
...
function InitDT_Filter(element, startDate, endDate) {
    ...
    var e = $(element).dataTable({
        ...
        bRetrieve: true,
        sPaginationType: "bootstrap",
        sAjaxSource: "/api/services.asmx/admin_dtShippingDistribution_FilterDate",
